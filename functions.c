...
void ktab(char *katba, char loun)
{
	...
	while (*katba != 0){
		...
		*video_ram = loun; //on met la valeur de la couleur dans l'emplacement adéquat
		
		if (*katba != '\n') {
			*video_ram++; //et on avance dans la mémoire
		}
		if (*katba == '\n') {
			video_ram += hsb*2+1; //s'il y a retour à la ligne on avance de sorte compter tous les emplacements qu'on a laissé vides
			hsb = LINE_SIZE;
			hsb_tani--;
